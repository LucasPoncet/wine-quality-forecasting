...
class FeatureTokenizer(nn.Module):
    ...
    def __init__(self, num_num_features: int, cat_sizes: dict[str, tuple[int, int]], d_model: int):
        super().__init__()
        ...
        self.cat_embs = nn.ModuleDict(
            {name: nn.Embedding(vocab, d_model) for name, (vocab, _d) in cat_sizes.items()}
        )
